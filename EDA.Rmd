...
boxplot_df = 
  ...
  head(400) %>%
 ggplot(aes(x = rating, y = genres),color = genres) + geom_boxplot()
boxplot_df 
...
boxplot_df2 = 
  merge(filter_ratingscore, movie_names) %>%
  head(500) %>%
  ggplot(aes(x = rating, y = year),color = year) + geom_boxplot()+
...
tag_review_plot =
  ...
  head(30) %>%
  ggplot(aes(x = rating, y = tag),color = tag) + geom_boxplot()+
  viridis::scale_fill_viridis(discrete = TRUE)
tag_review_plot
