...
- **Key steps** : We first merge the "high_rating" and "movie_names" dataframes, and count the "movie_id" variable and filter to keep the the rating times higher than 100 times movies. Next, arrange the n to decreasing-wise rank and seperate the year of movie publication of the variable called "title" and create a new variable called "year". Finally, show the results in a tidy table.
...
- **Key steps** : We first create a new dataframe called "movie_names_ave". For this dataframe, we use the 
"movies.csv" dataset and did the data cleaning as we mentioned in the previous section and also exclue the "no genres listed" for the "genres" variable.Next, we merge the "filter_ratingscore" and "movie_names_ave" dataframes, and calculate the mean rating score for each years and each genres. Finally, we show the first 10 lines of result in a tidy table.
```{r,message = FALSE,warning = FALSE}
movie_names_ave = 
  read_csv("./data/small/movies.csv") %>% 
  rename(movie_id = movieId) %>% 
...
- **Key steps** : We did the data cleaning process as we mentioned in the previous data cleaning section. Next,we merge the "filter_ratingscore" and "movie_names" dataframes, clean the names to lower cases, seperate "genres" variable. Finally, we create boxplot and let the x equals rating, and y equals genres.
...
- First, we checked whether the rating is normally distributed by histogram.
