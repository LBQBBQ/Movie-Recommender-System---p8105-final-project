...
As for further analysis, we chose to clean some of our datasets. For dataset "rating.csv", we applied the `janitor::clean_names()` function to make sure that everything is in lower case, and we also excluded the "timestamp" variable that did not related to our analysis assumption. After this step, we set up a new data frame called "rating_tidy".  Next up, we also created a new dataframe called "high_rating", which only contains the highest rating score "5" of the "rating_tidy" dataframe. Last but not the least, we renamed the variable "movieId" to movie_id for the "movie.csv" dataset and transfered it to a new data frame called "movie_names". Other trivial data cleaning process were mentioned in the following EDA section.
...
We first imported the dataset:"ratings.csv", and applied `janitor::clean_names()` to made everything into lower cases. Next, we applied the `summary()` function to get the summary table and finally showed the result.According to the result, variable rating showed that the minimum is 0.5, the first quantile is 3, median is 3.5, mean is 3.502, third quantile is 4 and the maximum is 5. Since userid, movieid, timestamp only represented the ID of users, id of movies respectively, and digital record of the time of occurrence of a particular movie, we chose not to analyze those three variables. 
```{r, message = FALSE, warning = FALSE}
...
We first imported the dataset:"movies.csv", and applied `janitor::clean_names()` to made everything into lower cases. We also applied `separate()` function to separate the year of the variable "title".  Next, we applied counted the genres by ` count() ` function. We also applied the `filter()` function to keep the n number lager than 1500. For the dataframe, we did the same steps mentioned above and finally we should the tidy table results The result were as following. Drama appeared 4361 times,1st place. Comedy appeared 3756 times, 2nd place. Thriller appeared 1894 times, 3rd place. Action appeared 1828 times, 4th place.Romance appeared 1596 time, 5 th place. Next, we also counted the 5 most appeared year of the movie released, and made a table which shoes the result. 2006 appeared 733 times,1st place. 2009 appeared 719 times, 2nd place. 2005 appeared 709 times, 3rd place. 2002 appeared 705 times, 4th place. 2008 appeared 691 time, 5 th place.
```{r, message = FALSE, warning = FALSE}
...
We first merged the "high_rating" and "movie_names" dataframes, and counted the "movie_id" variable and filtered to keep the the rating times higher than 100 times movies. Next, arranged the n to decreasing-wise rank and separated the year of movie publication of the variable called "title" and created a new variable called "year". Finally, showed the results in a tidy table. The Shawshank Redemption (1994) was the most popular movie, which had 153 times user rating. No.2 Pulp Fiction (1994),  had 123 times user rating. 116 times user rating, Forrest Gump (1994) won the 3rd popular place. The Matrix (1999) had 109 times user rating,and was the fourth place among the top 5 popular movies. No. 5 was Star Wars: Episode IV - A New Hope (1977), and it had 104 times user rating. We also found out that among those 5 most rated movies and highest rating movies, genres with tag drama appeared the most(3 times, appear in Forrest Gump (1994), Pulp Fiction (1994),The Shawshank Redemption (1994)). We also found out one interesting fact was that among the 5 most popular movies and 5 highest rating movies, 3 movies were released in 1994; 4 movies were released in the 90s.
```{r, message = FALSE, warning = FALSE}
data_path = "./data/small/ratings.csv"
...
As to find out the average rating of each genres in different year, we first `separate()` the combined genre and found out the average rating in different by using `group_by()` and `summarize()` functions. The table clearly showed that in 1902, the average ratings for genres Action, Adventure, Fantasy, Sci-Fi were all 3.5. In 1903, the average ratings for genres Crime and Western were both 2.5. The averages for genres Animation, Comedy, Sci-Fi in 1908 were all 4.0. The genre Drama's average ratings in 1915 was 2.0.
```{r,message = FALSE,warning = FALSE}
movie_names_ave = 
  read_csv("./data/small/movies.csv") %>% 
  rename(movie_id = movieId) %>% 
...
We first decided to do a test to determine whether there were any statistically significant differences between the mean ratings of the user from the 600 users by using ANOVA. In order to use ANOVA, we checked whether the rating was normally distributed or not. However, because the plot showed that the rating was not normally distributed, we needed to use non parametric test.Thus, we used the Kruskal-Wallis Test regarding user ID and rating. The Kruskal-Wallis H test (also known as the "one-way ANOVA on ranks") is a rank-based nonparametric test that may be used to see if two or more groups of an independent variable on a continuous or ordinal dependent variable have statistically significant differences. It is the nonparametric counterpart of the one-way ANOVA (Kruskal-Wallis H Test in SPSS Statistics | Procedure, output and interpretation of the output using a relevant example., 2021). Our assumption for KW test were: $H_0:\mu_{0}=\mu_1=\mu_2=\mu_3=...=\mu _x$,$H_1: \text{At least two of the mean ratings of the users are different}$. The result showed that the p value was less than 0.05. We can thus concluded that at 0.05 significance level, we rejected the null hypothesis and concluded that at least two of the mean ratings of the user from the 600 users were different.
```{r}
...
We did the data cleaning process as we mentioned in the previous data cleaning section. Next,we merged the "filter_ratingscore" and "movie_names" dataframes, cleaned the names to lower cases, seperateed "genres" variable. Finally, we created boxplot and let the x equaled rating, and y equaled genres.The boxplot regarding ratings and genres showed that the spread of the ratings of War, Mystery,Horror and Crime were larger comparing to the other ratings of genres. All of ratings had outliers around either 1.0 point or 0.5 point. The ratings of genres Film-Noir and documentary had outliers at 1.5 points.We can also conclude that the mean of the ratings of genres like Thriller,sci-fi, Romance, Musical, Horror, Fantasy, Comedy, Children, Adventure and Action all equaled to 3.5. The mean of the ratings of genres included Western, War, Mystery, IMAX, Film-Noir, Drama, Documentary, Crime and Animation all equaled to 4. 
...
We did the data cleaning process as mentioned above and merged the "filter_ratingscore" and "movie_names". Next, we removed the braket of the "year" variable, and showed only the first 1000 data. Finally, we set the x variable as rating and y equals to year, and showed the boxplot of the rating distribution among different years.Since this dataset included too much year, we chose to show only part of the boxplot regarding year and rating. According to the boxplot, we can conclude that the rating of years including 2010, 2009, 2007,2006 and 1998 had the least rating data records. There were no obivious trend shown in the plot.
...
3. For a user, firstly find his commonly used tags, and then find the most popular movies with these tags to recommend to the user.
...
The difficulty of the function is to find the correct sets of tags or users. To achieve the goal, we use the knowledge we have learned in the class, manipulate the dataset count the corresponding numbers we need during each trial. Sum all of the value we got in each trial and then we can get the interest value of the user on a specific movie (or item). The function code can be seen [here](https://shaocongz.github.io/Final_Project/tag_system_recommendation.html#Construct_interest_function).

&nbsp;

After building the function, we want to select the most popular movies to see how this function works. That is, we want to visualize the interest value. However, the dataset only contains about 3700 tags labeled by the users. The limited dataset will destroy the credibility of the tag-based recommendation system, as the user we want to analyze may have a few tags and there may also be a few tags for movies. In order to make sure that the interest values to be nonzero, we will then choose several movies that have been labeled most by users and select several active users. In this way, we select the users that have labeled more than 20 movies and select movies that have been labeled by more than 4 users. The selected movies, for example, Star Wars: Episode IV - A New Hope. The selected movies' table is [here](https://shaocongz.github.io/Final_Project/tag_system_recommendation.html#Find_the_most_popular_movies_and_the_most_active_users).

&nbsp;

To visualize the users' interest values, we decided to create a heat map. A heat map (or heatmap) is a data visualization technique that shows the magnitude of a phenomenon as color in two dimensions. The variation in color may be by hue or intensity, giving obvious visual cues about how the phenomenon is clustered or varies over space. The heat map is shown below. The horizontal coordinate of the heat map is the movie ID and the vertical coordinate is the user ID, each corresponding color represents the level of the user's interest in the movie. The brighter the color (the greater the interest value), the deeper the interest, and the darker the color (the smaller the interest value), the lighter the interest. The detailed code is [here](https://shaocongz.github.io/Final_Project/tag_system_recommendation.html#Create_heat_map).
